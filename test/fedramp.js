...
var self=this;var mapping={'Created_At':'lastRefresh','Produced_By':'producedBy'};/**
         ...
         */function today(date){var d=date;var dd=d.getDate();var mm=d.getMonth()+1;var yyyy=d.getFullYear();if(dd<10){dd='0'+dd;}if(mm<10){mm='0'+mm;}return mm+'/'+dd+'/'+yyyy;}return self.init(options);}})();(function(){'use strict';angular.module('fedramp.components').component('agenciesGrid',{templateUrl:'templates/components/agencies-grid.html',controller:AgenciesGrid,controllerAs:'controller',bindings:{rawItems:'<?',onUpdate:'&?'}});AgenciesGrid.$inject=['$log','fedrampData','$attrs'];/**
     * @constructor
     * @memberof Components
...
if(!searchTerm){return product;}searchTerm=searchTerm.toLowerCase();var productName=product.name.toLowerCase();var providerName=product.provider.toLowerCase();if(productName.indexOf(searchTerm)!==-1||providerName.indexOf(searchTerm)!==-1){return product;}return null;};/**
     ...
     * @constructor
     * @memberof Components
     */function Twitter(){var script=document.createElement('script');script.async='async';script.src='https://platform.twitter.com/widgets.js';script.charset='utf-8';document.body.appendChild(script);}})();(function(){'use strict';angular.module('fedramp.services').factory('Cache',CacheFactory);CacheFactory.$inject=['$cacheFactory'];/**
     * @constructor
     * @memberof Services
...
// agency information.
// Top level
for(var i=0;i<data.length;i++){var _d=data[i];if(validAgency(_d.sponsoringAgency)&&include(_d.sponsoringAgency,names)){names.push(_d.sponsoringAgency.trim());var _item=new Agency();_item.name=_d.sponsoringAgency.trim();_item.logo=_d.sponsoringAgencyLogo.trim();_item.pocName=safeTrim(_d.agencyPocName);_item.pocEmail=safeTrim(_d.agencyPocEmail);items.push(_item);}if(validAgency(_d.authorizingAgency)&&include(_d.authorizingAgency,names)){names.push(_d.authorizingAgency.trim());var _item2=new Agency();_item2.name=_d.authorizingAgency.trim();_item2.logo=_d.sponsoringAgencyLogo.trim();_item2.pocName=safeTrim(_d.agencyPocName);_item2.pocEmail=safeTrim(_d.agencyPocEmail);items.push(_item2);}if(validAgency(_d.authorizingSubagency)&&include(_d.authorizingSubagency,names)){names.push(_d.authorizingSubagency.trim());var _item3=new Agency();_item3.name=_d.authorizingSubagency.trim();_item3.logo=_d.sponsoringAgencyLogo.trim();_item3.pocName=safeTrim(_d.agencyPocName);_item3.pocEmail=safeTrim(_d.agencyPocEmail);items.push(_item3);}}// Nested
